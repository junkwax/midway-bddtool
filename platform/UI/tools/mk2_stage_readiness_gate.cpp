...
void draw_mk2_stage_readiness_gate(void)
{
    ...
    if (ImGui::BeginTable("readiness_gate", 3, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg)) {
        ...
        snprintf(detail, sizeof detail, "%d misaligned-width sprite(s)%s%s",
                 d.odd_width_images,
                 d.odd_width_label[0] ? ", first " : "",
                 d.odd_width_label[0] ? d.odd_width_label : "");
        draw_gate_row("Sprite width", even_width_ok, detail,
            "Sprite widths must be a multiple of 4 or the art shears progressively in game: LOAD2 scans row headers on a 4-byte-padded stride but copies payload from the tight rows. Pad each one out with transparent columns. All 1081 shipped background images are 4-aligned; even is not enough.");

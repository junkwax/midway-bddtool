...
int bdd_viewer_run_cli_command(int argc, char **argv, int *exit_code)
{
    ...
    *exit_code = 0;
    ...
    if (argc >= 2 && strcmp(argv[1], "--stage-actors") == 0) {
        ...
        for (int i = 0; i < n; i++) {
            fprintf(stderr, "  proc=%-18s seq=%-16s frames=%d pos=%d motion=%.2fpx/f baklst=%d scroll=%.4f%s\n",
                    actors[i].proc, actors[i].sequence, actors[i].frame_count,
                    actors[i].pos_count, actors[i].motion_x / 65536.0,
                    actors[i].insert_baklst, actors[i].scroll,
                    actors[i].screen_anchored ? " screen-anchored" : "");
